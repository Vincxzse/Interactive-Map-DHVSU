...
function attachMRM202Entrance1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.mrm202Entrance1, () => {
        scene.loadMRM202(200, 150)
        scene.refreshDebug()
    })
}

function attachMRM202Entrance2Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.mrm202Entrance2, () => {
        scene.loadMRM202((scene.worldWidth / 2) - 200, 150)
        scene.refreshDebug()
    })
}

function attachMRM202Exit1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM202Exit1, () => {
        scene.loadMRM2nd(930, (scene.worldHeight / 5) - 100)
        scene.refreshDebug()
    })
}

function attachMRM202Exit2Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM202Exit2, () => {
        scene.loadMRM2nd(1130, (scene.worldHeight / 5) - 100)
        scene.refreshDebug()
    })
}
...
function attachMRM204Entrance1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM204Entrance1, () => {
        scene.loadMRM204(125, 200)
        scene.refreshDebug()
    })
}

function attachMRM204Exit1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM204Doormat1, () => {
        scene.loadMRM2nd(scene.worldWidth - 75, 507)
        scene.refreshDebug()
    })
}

function attachMRM205Entrance1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM205Entrance1, () => {
        scene.loadMRM205(125, 200)
        scene.refreshDebug()
    })
}

function attachMRM205Entrance2Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM205Entrance2, () => {
        scene.loadMRM205(125, (scene.worldHeight / 3.5) - 100)
        scene.refreshDebug()
    })
}

function attachMRM205Exit1Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM205Doormat1, () => {
        scene.loadMRM2nd((scene.worldWidth - 75), 95)
        scene.refreshDebug()
    })
}

function attachMRM205Exit2Overlap(scene) {
    scene.currentOverlap = scene.physics.add.overlap(scene.player, scene.MRM205Doormat2, () => {
        scene.loadMRM2nd((scene.worldWidth - 75), 345)
        scene.refreshDebug()
    })
}
